...
//hàm cập nhật số lượnggit sản phẩm trong giỏ hàng và giao diện
...
const cartItemHTML = cart.map((cartItem) => {
  return axios({
    url: `${api}/${cartItem.id}`,
    method: "GET",
  })
    .then((res) => {
      const cartPro = res.data;
      return `
...
Promise.all(cartItemHTML)
  .then((htmlArray) => {
    // Nối tất cả các đoạn mã HTML thành một chuỗi HTML duy nhất
    const contentHTML = htmlArray.join("");

    // Thêm chuỗi HTML vào label
